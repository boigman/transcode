...
#
import MythTV
import MythTV.ttvdbv4.ttvdbv4_api
...
def runjob(jobid=None, chanid=None, starttime=None, tzoffset=None):
    ...
    utcstarttime = utcstarttime + timedelta(hours=tzoffset)  # adjusted for DST

    if debug > 0:
        print('chanid "%s"' % chanid)
        print('utcstarttime "%s"' % utcstarttime)

    rec = Recorded((chanid, utcstarttime, subtitle), db=db);
    utcstarttime = rec.starttime;
    starttime_datetime = utcstarttime
    subtitle = rec.subtitle.replace(' ', '').replace(':', '').replace('!', '')
                ...
                if jobitem.type == jobitem.COMMFLAG:  # Commercial flagging job
                    ...
                    if jobitem.status == jobitem.RUNNING:  # status = RUNNING?
                        job.update({'status': job.PAUSED,
                                    'comment': 'Waited %d secs for the commercial flagging job' % (
                                                waititer * POLL_INTERVAL) \
    ...
    logfile = os.path.join(sg.dirname + '/log/', rec.basename.replace('.ts','.log'))
    ...
    outfile = os.path.join(sg.dirname + '/Transfer/',
                           rec.title.replace('!', '') + rec.starttime.strftime('-%Y-%m-%d-%H') + (
                               '-' + subtitle if subtitle != "" else '') + '_part.mp4')
    if debug > 0:
    ...
    if estimateBitrate:
        if jobid:
            job.update(
                {'status': job.RUNNING, 'comment': 'Estimating bitrate; detecting frames per second, and resolution.'})

        duration_secs, xstderr = get_duration(db, rec, transcoder, tmpfile);
        if duration_secs > 0:
    ...
    xcrf = crf
    #    if chanid in [1051]:
    # Channels ending with '1' seem to use higher bandwidth, so increase crf to accommodate
    if str(chanid).endswith('1'):
        xcrf = '30'
    if estimateBitrate:
        if isHD:
    ...
    with open(tmpstatusfile) as f:
        ...
        while t.is_alive():
            ...
            if len(lines) > 0:
                ...
                #                    print lines;
                hangiter = 0
                if len(lines) > 0 and lines[0].startswith('frame'):
                    ...
                    if debug > 0:
                        print('Read pathological output %s' % lines[0])
                else:
                    if debug > 0:
                        if hangiter > 0:
    ...
    duration_msecs, xstderr = get_duration(db, rec, transcoder, outfile)
    ...
    if jobid:
        if output_bitrate:
            job.update({'status': job.FINISHED,
                        'comment': 'Transcode Completed @ %dkbps, compressed file by %d%% (clipped %d%%, transcoder compressed %d%%)' % (
                        output_bitrate, int(compressed_pct * 100), int(clipped_compress_pct * 100),
                        int(actual_compression_ratio * 100))})
        else:
            job.update({'status': job.FINISHED, 'comment': 'Transcode Completed'})
    os.rename(outfile, outfile.replace('_part.mp4', '.mp4'))
...
def encode(jobid=None, db=None, job=None,
           ...
           abitrate_param='-c:a aac -b:a 128k',
...
def main():
    tzoffset = time.localtime().tm_isdst - 6
    ...
    parser.add_option('-b', '--batch', action='store', type='string', dest='batch',
                      help='Batch Processing')
    parser.add_option('--vidfile', action='store', type='string', dest='vidfile',
                      help='Enter video (.ts) filename')
    ...
    if opts.batch:
        ...
        # sd = MythTV.StorageGroup
        # db2 = MythDB()
        # if db2 is None:
        #     print('MythTV database not found')
        #     sys.exit(0)
        sgx = requests.get('http://localhost:6544/Myth/GetStorageGroupDirs')
        pattn = re.compile('\<DirName\>(.*)\</DirName\>')
        m = pattn.search(sgx.text)
        if(len(m.groups())>0):
            storgroupdir = m.group(1)
            print('Storage Dir='+ storgroupdir)
            dir_name = storgroupdir + '/Transfer/'
...
#            for file_name in list_of_files:
                myindex = file_name.find('_part.mp4')
                if file_name.find('_part.mp4') < 0:
                    list_of_files.remove(file_name)
#                    i -= 1
                    continue
            ...
            if len(list_of_files)>0:
                print("Transcoding may be in progress: " + file_path + " last modified at: " + \
                      time.ctime(os.path.getctime(file_path)))
                sys.exit(0)

            for root, dirs, files in os.walk(storgroupdir):
                for file in files:
                    if file.endswith(".ts") and os.path.exists(os.path.join(root, file + '.png')):
                        print(os.path.join(root, file))
                        vidfile = file
                        chanid = vidfile.split('_', 1)[0]
                        starttime = vidfile[len(chanid) + 1:].replace('.ts', '')
                        runjob(chanid=chanid, starttime=starttime, tzoffset=tzoffset)
#                        sys.exit(0)
        sys.exit(0)

    if opts.vidfile:
        vidfile = opts.vidfile
        chanid = vidfile[0:4]
        starttime = vidfile[5:].replace('.ts', '')
        runjob(chanid=chanid, starttime=starttime, tzoffset=tzoffset)
    else:
        if len(args) == 1:
            vidfile = args[0]
            if vidfile.endswith('.ts'):
                #                chanid = vidfile[0:4]
                #		 starttime=vidfile[5:].replace('.ts','')
                chanid = vidfile.split('_', 1)[0]
                starttime = vidfile[len(chanid) + 1:].replace('.ts', '')
            ...
            runjob(chanid=opts.chanid, starttime=opts.starttime, tzoffset=tzoffset)
        else:
